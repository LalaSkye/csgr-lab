...
class ClauseSpec(BaseModel, frozen=True):
    ...
    clause_id: ClauseId = Field(default_factory=_new_id)
...
class ContractSpec(BaseModel, frozen=True):
    ...
    contract_id: ContractId = Field(default_factory=_new_id)
    ...
    @classmethod
    def _no_duplicate_clause_ids(cls, v: tuple[ClauseSpec, ...]) -> tuple[ClauseSpec, ...]:
